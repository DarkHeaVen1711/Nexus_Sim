...
static double bench_naive(const std::vector<BenchPoint>& pts,
                          double radius, int queries) {
    std::mt19937 rng(123);
    std::uniform_real_distribution<double> dist(0.0, 10000.0);
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int q = 0; q < queries; ++q)
        naive_radius_count(pts, dist(rng), dist(rng), radius);
...
int main() {
    ...
    for (size_t n : counts) {
        ...
        double t_naive = bench_naive(pts, radius, queries);
        double t_qt    = bench_quadtree(pts, radius, queries);
